...
Notice the use of ```ts z.trim()``` and ```ts z.toLowerCase()```, Zod allows schemas to preprocess and post-process
payloads to meet specific requirements without throwing an error. In the case, email addresses that start or end with
spaces could be refined as needed, but not throwing an error and make users frustrated.

TypeScript types are inferred from Zod schemas via ```ts z.infer<typeof ...>```, and exported for use of both the
...
== Demonstration
It should be noted that images might not be not up-to-date, as the report is authored during development. However, it
would not deviate significantly from the end result of Part I.
...
description (typos could also be caught and handled).
...
```html <Button pill>Hello World!</Button>```.
